...
export class DownloadMyFormatDJComponent {
  ...
  prdsId: number = 0
  month: number | undefined = MONTHS.find(e => e.id == GlobalHelpers.monthBefore().getMonth() + 1)?.id
  ...
  public openPDFNewWindow() {
    ...
    let params={tellId: this.tellId, bussState: this.bussState, prdsId:this.prdsId, month:this.month}
    GlobalHelpers.openWindowWithPost(environment.API_URL + `/api/v1/reports/my-format-dj-json`, params)
  }
  ...
  getDataPeriods(): boolean {
    ...
    this.periodService.all().subscribe({
      next: data => {
        ...
        let period = this.periods.find(e => Number(e.prdsNameShort) == GlobalHelpers.monthBefore().getFullYear())
      ...
      error: error => {
        this.showMessage.error({ message: 'Al parecer surgio un error!. ' + error.message.message })
      }
    })
    return false;
  }
