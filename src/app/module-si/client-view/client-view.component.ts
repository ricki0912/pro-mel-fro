...
export class ClientViewComponent implements OnInit, OnDestroy, CrudInterface {
  ...
  readCRUD(id: number[]): boolean {
    ...
    this.businessService.findBusiness(id)?.subscribe({
      next: (data) => {
        ...
        if (this.business[0].tellId) this.findTeller(this.business[0].tellId);
      },
      error: (e) => {
  ...
  findTeller = (tellId: number) => {
    this.tellerService
      .find(tellId)
      .subscribe({ next: (d) => this.clientViewService.onTeller(d) });
  };
  getTeller = () => {
    this.clientViewService.getTeller().subscribe({
      next: (d) => {
        if (d) {
          this.teller = d;
        }
  ...
  openDialogAnnualSumary() {
    //const id = [bussId || -1];
    const dialogRef = this.dialogEditClient.open(AnnualSummaryComponent, {
      panelClass: 'dialog',
      maxWidth: '100vw',
      maxHeight: '100vh',
      height: '100%',
  ...
  openDialogChangeState(
    ...
    const dialogRef = this.dialogEditClient.open(ChangeStateComponent, {
      panelClass: 'dialog',
      data: {
        row: { bussState: Number(bussState), bussStateDate: bussStateDate },
    ...
    this.businessService.updateBusinessTellId(bussIds, tellId).subscribe({
      ...
      error: (e) => {
        this.showMessage.error({
          ...
          action: () => this.updateBusinessTeller(bussIds, tellId),
        });
      },
    });
  }
  ...
  private updateBusinessState(
    bussIds: number[],
    bussState: number,
    bussStateDate: Date
  ) {
    this.bussinesService
      .updBusinessState(bussIds, bussState, bussStateDate)
      .subscribe({
        next: (d) => {
          this.showMessage.success({ message: d.msg });
        ...
        error: (e) => {
          this.showMessage.error({
            message: e.error.message,
            action: () =>
              this.updateBusinessState(bussIds, bussState, bussStateDate),
          });
        },
      });
  }
  ...
  private updateBusinessComment(bussIds: number[], bussComment?: string) {
    this.bussinesService.updBusinessComment(bussIds, bussComment).subscribe({
      ...
      error: (e) => {
        this.showMessage.error({
          message: e.error.message,
          action: () => this.updateBusinessComment(bussIds, bussComment),
        });
      },
    });
  }
